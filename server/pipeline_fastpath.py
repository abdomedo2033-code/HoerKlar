...
def run_fastpath(video_id, title, workdir, vocab=(), seed=41,
    ...
    # Sentence traps go FIRST — word traps only fit single-word answers and
    # must never pre-empt (and block) the sentence ones.
    try:
        import mt_ar
        ...
        if n_wt:
            print(f"[fastpath] +{n_wt} AR word-trap meanings", flush=True)
